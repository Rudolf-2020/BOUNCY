...
$('.goto').click(function () {
	...
	if ($('.header__menu').hasClass('active')) {
		$('.header__menu,.icon-menu').removeClass('active');
		$('body').removeClass('lock');
	}
	return false;
});
...
$('.portfolio-works').isotope({
	itemSelector: '.portfolio-works__image',
	layoutMode: 'masonry'
});


// filter, portfolio
var imagesbox = $('.portfolio-works');
var buttonItem = $('.portfolio-menu__bitton');

buttonItem.click(function(){
	var selesctFilter = $(this).data('filter');
	$('.portfolio-works').isotope({
		filter: selesctFilter
	});
	$('.portfolio-menu__bitton').removeClass('active');
	$(this).addClass('active');
});

//trigger
setTimeout(() => $('#triggerClick').trigger('click'), 50);
/*
Этот trigger необходим, дело в том что isotope.js слегка глючит, и иногда при открытии страницы 
или при ее перезагрузки, "плитки" схлопываются, они накладываются друг на друга. Но если покликать по 
элементам меню portfolio, то все становится на свои места, поэтому я сделал так что при открытии 
страницы генерируется событие click на элемент меню "All Works", через 50 ms, мгновенно нельзя 
иначе эффекта не будет. Итого: при загрузке страницы на указанный элемент меню генерируется 
событие click, и если плитки схлопнулись то сгенерированное событие поставит все на свои места
и при этом все будет выглядеть так как будто страница просто загрузилась, и никаких лагов нет.
*/

//Our Price
$('.price__item').hover(function(){
	$(this).find('.price-item__price').toggleClass('active');
	$(this).find('.price-item__title').toggleClass('active');
	$(this).find('.price-item__list').toggleClass('active');
	$(this).find('.price-item__link').toggleClass('active');
});


// Google-map, button

$('.google-map__text').click(function(){
	$('.google-map__button').hide();
});

$('.google-map__arrow').click(function(){
	$('.google-map__button').hide();
});
